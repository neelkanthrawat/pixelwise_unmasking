...
class MaskedMNIST(Dataset):
    def __init__(self, base_dataset, train=True, 
                 mask_type:Literal['square', 'random', 'checkerboard_pattern']='square',
    ...
    def __getitem__(self, idx):
        ...
        """
        image = self.base_dataset[idx] # imag, label, we dont care about label
    ...
    def apply_random_mask(self, image):
        ...
        mask[:, ys, xs] = 0
        masked_image[:, ys, xs] = -1 # some negative value (super low)
...
def get_masked_mnist_dataloaders(batchsize, pixelwidth, digit=None, zero_one_range=False):
    ...
    train_dataset = MaskedMNIST(train_base)
    test_dataset  = MaskedMNIST(test_base)
